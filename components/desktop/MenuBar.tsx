...
export default function MenuBar() {
  ...
  return (
    ...
    >
      ...
      >
        <motion.span variants={item} className="flex items-center text-white drop-shadow-sm">
          {logoOk ? (
        ...
        <motion.span variants={item} className="font-semibold tracking-tight">
          {BRAND.name}
        </motion.span>
        <ul className="hidden items-center gap-4 sm:flex">
          {menuItems.map((label) => (
            <motion.li
              key={label}
              variants={item}
              className="cursor-default rounded px-1 text-white/80 transition-colors duration-200 hover:bg-white/15 hover:text-white"
            >
              {label}
            </motion.li>
          ))}
